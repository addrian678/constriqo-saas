...
check("Runtime readiness valida migracion requerida", serverSource.includes("0064_attendance_daily_limit_payroll.sql") && serverSource.includes("MIGRATIONS_NOT_READY"), "readiness migration");
...
try {
  ...
  const readyServer = createRuntimeServer({
    async readinessProbe() {
      return { requiredMigration: "0064_attendance_daily_limit_payroll.sql" };
    },
  });
