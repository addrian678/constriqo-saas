...
check("Ruta recuperacion admin declarada", routes.includes("/api/super-admin/tenants/:tenantId/admins/:userId/reset-password"), "admin recovery route");
...
check("Cliente frontend recupera admin de tenant", client.includes("resetTenantAdminPassword") && page.includes("Clave + MFA"), "client recovery");
...
check("Panel usa estadisticas compactas", page.includes("super-admin-stats-grid") && styles.includes(".super-admin-stats-grid {\n  display: grid;") && styles.includes("repeat(4, minmax(0, 1fr))"), "compact stats");
...
check("Repository recupera admin sin exponer clave previa", repository.includes("resetTenantAdminPassword") && repository.includes("passwordStored: \"argon2id-hash-only\"") && repository.includes("auth_sessions SET status = 'revoked'"), "safe admin recovery");
